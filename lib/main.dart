...
class System extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ChangeNotifierProvider<ChatProvider>(create: (BuildContext _) => ChatProvider()),
          ...
          if(route.name == ChatRoom.path) {
            return MaterialPageRoute(
                ...
                builder: (BuildContext context) {
                  ...
                  return ChatRoom(chatProvider: chatProvider, userProvider: userProvider,);
                }
            );
          }
          return MaterialPageRoute(
              ...
              builder: (BuildContext context) {
                ChatProvider chatProvider = Provider.of<ChatProvider>(context);
                UserProvider userProvider = Provider.of<UserProvider>(context);
                return Login(chatProvider:chatProvider, userProvider: userProvider);
              }
          );
        },
      ),
...
class Login extends StatefulWidget {
  ...
  const Login({Key? key, required this.chatProvider, required this.userProvider}) : super(key: key);
...
class _LoginState extends State<Login> {
  ...
  Future<void> _move(BuildContext context) async{
    ...
    final bool _connectCheck = await widget.chatProvider.join(nickName: _controller.text);
    if(!_connectCheck){
      await showDialog<void>(
          context: context,
          builder: (BuildContext context) => AlertDialog(
...
class ChatRoom extends StatefulWidget {
  ...
  final ChatProvider chatProvider;
  final UserProvider userProvider;
  const ChatRoom({Key? key, required this.chatProvider, required this.userProvider}) : super(key: key);
...
class _ChatRoomState extends State<ChatRoom> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("채팅"), centerTitle: true,),
      body: Stack(
        children: [
          Container(
            child: Column(
              children: [
                Expanded(
                    child: Container(
                      child: widget.chatProvider.chat.isEmpty ? Container() : ListView.builder(
                          controller: _sc,
                          reverse: true,
                          ...
                          itemBuilder: (BuildContext context, int index){
                            ...
                            final ChatModel _chatModel = widget.chatProvider.chat[index];
                            final String _userNickName = widget.userProvider.userNickName;
                            if(_chatModel.userNickName != _userNickName) {
                              return Container(
                                    ...
                                    child: Container(
                                      ...
                                      child: Container(
                                          ...
                                          child: Text.rich(
                                            TextSpan(
                                                children: [
                                                  TextSpan(text: "${_chatModel.userNickName} : ", style: const TextStyle(fontWeight: FontWeight.bold),),
                                                  TextSpan(text: "${_chatModel.chat}")
                                                ]
                                            ),
                                          )
                                      ),
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        ...
                        IconButton(onPressed: () => setState(() => isOpen = !isOpen), icon: Icon(!isOpen ? Icons.arrow_drop_down : Icons.arrow_drop_up)),
                      ],
                    ),
                    !isOpen ? Container() : ListView(
                      shrinkWrap: true,
                      children: widget.chatProvider.chatUserList.map<Widget>(
                              (String userNickName) => Container(
  ...
  void _send(){
    ...
    widget.chatProvider.sendChat(nickName: (widget.userProvider.userNickName), chat: _tc.text);
    _tc.clear();
  }
}
  ...
  Future<bool> join({required String nickName}) async{
    ...
    mqttService.client.updates?.listen((List<MqttReceivedMessage<MqttMessage>> event) {
      ...
      if(_json["join"] != null){
        ...
        final ChatModel _chatModel = ChatModel.json(_json);
